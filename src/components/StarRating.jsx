    ...
    <>
      ...
      >
        ...
        <defs>
          ...
          <linearGradient id={`grad${filled}`} gradientTransform="rotate(0)">
            {/* This line is a from 0 - filled */}
            <stop offset={`${filled * 100}%`} stopColor="#DEF81C" />
            {/* This line is a from filled - 100 */}
...
const StarRating = ({ rating, maxRating = 5 }) => {
  // Create an array of stars based on max rating
  const stars = Array.from({ length: maxRating }, (_, index) => {
    const filledValue = Math.min(Math.max(rating - index, 0), 1); // Handles fractional values
